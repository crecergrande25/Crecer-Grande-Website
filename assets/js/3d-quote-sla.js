(()=>{
  ...
  const reviewBreakdown='<div><span>Turnaround</span><b>Within 24 hrs</b></div><div><span>Review</span><b>CAD + manufacturability</b></div><div><span>Output</span><b>Approx. price / recommendation</b></div>';
  ...
  function enforceReviewUI(){
    ...
    setText(quotePrice,'Within 24 hrs');
    setText(quoteGst,'Engineering-reviewed result after successful file submission.');
    if(quoteBreakdown&&quoteBreakdown.innerHTML!==reviewBreakdown)quoteBreakdown.innerHTML=reviewBreakdown;
    setText(fileState,'File accepted for Crecer Grande engineering review. Your result will be provided within 24 hrs of successful submission.');
    setText(slicerStatus,'This format is handled through the 24-hour engineering-review route. STL remains the instant-result format.');
  }

  function setMode(){
    const file=fileInput.files?.[0];
    if(!file){reviewMode=false;calcBtn.style.display='';sendBtn.style.display='';reviewBtn.style.display='none';return;}
    reviewMode=extOf(file)!=='stl';
    if(reviewMode){
      enforceReviewUI();setTimeout(enforceReviewUI,400);setTimeout(enforceReviewUI,1500);
    }else{
      calcBtn.style.display='inline-flex';sendBtn.style.display='inline-flex';reviewBtn.style.display='none';
      setText(fileState,'STL selected. Instant geometry analysis and FDM slicing are available.');
      setText(slicerStatus,'STL instant-result mode: configure the print and calculate.');
    }
  }

  const summary=document.querySelector('.q3d-summary');
  if(summary)new MutationObserver(()=>{if(reviewMode)setTimeout(enforceReviewUI,0)}).observe(summary,{subtree:true,childList:true,characterData:true});
