...
This is the protected V2.1 user-management backend used by Website Manager → Users & Access.
...
Deploy under the function name `admin-users` if it is not already deployed in the live project. No database migration is required for V2.1. Existing admin identities use the internal login domain `admin.crecergrande.in`; the function defaults to that domain and can be overridden with `ADMIN_AUTH_DOMAIN` if needed. Supabase normally provides `SUPABASE_URL`, `SUPABASE_ANON_KEY` and `SUPABASE_SERVICE_ROLE_KEY` to Edge Functions; verify the project environment before deployment.
